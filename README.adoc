...
* https://gravitino.apache.org/[Apache Gravitino] - A high-performance, geo-distributed, and federated metadata lake that provides unified metadata access for various data sources (filestores, relational databases, event streams) and AI assets. It supports multiple compute engines like Trino, Spark, and Flink, enabling end-to-end data governance with features like access control, auditing, and discovery.
